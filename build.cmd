...
::: Extract version
set VER=
for /f "tokens=3 usebackq" %%f in (`type brightly.rc ^| findstr /R /C:"#define[ ]VER_MAJOR"`) do set VER=%%f
for /f "tokens=3 usebackq" %%f in (`type brightly.rc ^| findstr /R /C:"#define[ ]VER_MINOR"`) do set VER=%VER%.%%f
for /f "tokens=3 usebackq" %%f in (`type brightly.rc ^| findstr /R /C:"#define[ ]VER_BUILD"`) do set VER=%VER%.%%f
rem for /f "tokens=3 usebackq" %%f in (`type brightly.rc ^| findstr /R /C:"#define[ ]VER_REVISION"`) do set VER=%VER%.%%f
...
IF EXIST "%ProgramFiles%\Microsoft Visual Studio\%%f\%%g\%%h\VC\Auxiliary\Build\vcvarsall.bat" SET "VCVARSALL=%ProgramFiles%\Microsoft Visual Studio\%%f\%%g\%%h\VC\Auxiliary\Build\vcvarsall.bat"
...
SET NOLOGO=/nologo
ECHO Compiling...
cl %NOLOGO% -c /EHsc /DUNICODE /D_UNICODE /Tc"brightly.c" /Tc"monitor.c"
IF ERRORLEVEL 1 GOTO ERROR
ECHO Resources...
rc %NOLOGO% brightly.rc
IF ERRORLEVEL 1 GOTO ERROR
ECHO Linking...
rem /manifest:embed  -- now external .manifest is included in .rc file
link %NOLOGO% /out:brightly.exe brightly brightly.res monitor /subsystem:windows
IF ERRORLEVEL 1 GOTO ERROR
ECHO Done: V%VER%
